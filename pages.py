...
<div class="login-wrap">
  <div class="login-card">
    ...
    <div class="login-hint">
      ...
      <span class="hint-value" onclick="document.getElementById('pw').value='123456';document.getElementById('pw').focus()">••••••</span>
    </div>
    <form id="form">
...
async function loadLogs() {
  try {
    ...
    el.innerHTML = logs.map(function(l) {
      ...
      var icon = icMap[level] || 'ti-info-circle';
...
async function loadDashboard() {
  try {
    ...
    document.getElementById('s-traffic').innerHTML = (d.total_traffic_mb || 0).toFixed(1) + ' MB';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setInterval(function() {
      var loaders = { dash: loadDashboard, users: loadUsers, configs: loadConfigs, traffic: loadTraffic, logs: loadLogs, settings: loadSettings, worker: loadWorker };
...
(function loadChartJS() {
  ...
  script.onload = function() { window.chartJsLoaded = true; if (currentPage === 'traffic') loadTraffic(); };
...
def get_public_page_html(uuid_key: str) -> str:
    ...
    safe_key = _html.escape(str(uuid_key))
...
<script>
var UUID_KEY='{safe_key}',savedPw='',currentData=null;
